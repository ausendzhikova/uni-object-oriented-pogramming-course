...
void Duelist::duel(Duelist& other) {
	unsigned int firstDuelistPoints = 0;
	unsigned int secondDuelistPoints = 0;
	if (deck.getTotalCardsCount() != other.deck.getTotalCardsCount()) {
		std::cout << "This duel is impossible!\n";
		return;
	}
	deck.shuffle();
	other.deck.shuffle();
	for (size_t i = 0; i < deck.getTotalCardsCount(); i++) {
		if (*deck.getCards().at(i) > *other.deck.getCards().at(i)) {
			firstDuelistPoints++;
		}
		else if (*deck.getCards().at(i) < *other.deck.getCards().at(i)) {
			secondDuelistPoints++;
		}
	}
