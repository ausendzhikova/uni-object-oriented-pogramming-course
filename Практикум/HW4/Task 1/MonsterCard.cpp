...
MonsterCard::MonsterCard(std::string name, std::string effect, unsigned int rarity, unsigned int attackPoint, unsigned int defencePoints) : Card(name, effect, rarity) {
...
std::istream& operator>>(std::istream& in, MonsterCard& obj) {
	...
	in >> obj.rarity >> c >> obj.attackPoints >> c >> obj.defencePoints;
	in.ignore(1, '\n');
	return in;
}
